 ...
 *
 * @WebformHandler(
 *   id = "medicare_forms_api_seminar",
 *   label = @Translation("MMSS - Medicare Forms API Seminar"),
 ...
 * )
 */
class WebformFormAPIHandlerSeminar extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    $logger = $this->logger->get('WebformFormAPIHandlerForSeminar - postSave');
    if ($update) {
      $logger->notice('WebformFormAPIHandlerForSeminar - updatesubmission');
    }
    ...
    $this->processFormDataSeminar($webform_submission, $getAccessToken['response'], $logger, "SEMINAR", $this->requestStack->fromOptions(), $apiKeys);
  }
   ...
   *   Boolean.
   */
  private function processFormDataSeminar(WebformSubmission $webform_submission, array $getAccessToken, LoggerChannel $logger, string $from_type, Client $requestStack, array $apiKeys): bool {
    $body_array_returned = $this->fieldToApiDataParsingSeminar($webform_submission, $logger, $from_type);
    $body_array_returned_json = Json::encode($body_array_returned);
    $logger->info("Form Data JSON Seminar- " . $body_array_returned_json);
    try {
      ...
      if (in_array($form_data_api_response_decoded['status'], [
        ...
        $webform_submission->setElementData('documentum_id', $form_data_api_response_decoded['id']);
        $webform_submission->resave();
      }
    }
    catch (\Exception $e) {
      $logger->info("Seminar - RequestException - " . $e->__toString() . "-getBody-" . $e->getMessage());
    }
    return TRUE;
  }
   ...
   *   Body Composed Array.
   */
  private function fieldToApiDataParsingSeminar(WebformSubmission $webform_submission, LoggerChannel $logger, string $from_type): array {
    ...
    $medicare_field_data_mapping_AOR_MAPD_exploded = explode(PHP_EOL, $bcbsma_webform_medicare_forms_api->get('medicare_field_data_mapping_seminar'));
    foreach ($medicare_field_data_mapping_AOR_MAPD_exploded as $val) {
      ...
      if ($val_explode['0'] == "memberSubmissionDate") {
        $body_composed_array['formData'][$val_explode['0']] = date('Y-m-d\TH:i:s.v\Z', (int) $webform_submission->getCreatedTime());
        continue;
      }
      if ($val_explode['0'] == "seminarVenue") {
        $body_composed_array['formData'][$val_explode['0']] = html_entity_decode($webform_submission->getElementData(trim($val_explode['1'])));
        continue;
      }
      if ($val_explode['0'] == "seminarType") {
        $body_composed_array['formData'][$val_explode['0']] = "";
        $seminar_type_id = $webform_submission->getElementData(trim($val_explode['1']));
        if (isset($seminar_type_id)) {
          if (is_object($this->entityTypeManager->getStorage('taxonomy_term')->load($webform_submission->getElementData(trim($val_explode['1']))))) {
            $body_composed_array['formData'][$val_explode['0']] = $this->entityTypeManager->getStorage('taxonomy_term')->load($webform_submission->getElementData(trim($val_explode['1'])))->getName();
          }
        }
        continue;
      }
