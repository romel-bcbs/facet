 ...
 *
 * @WebformHandler(
 *   id = "medicare_forms_api_webinar",
 *   label = @Translation("MMSS - Medicare Forms API Webinar"),
 ...
 * )
 */
class WebformFormAPIHandlerWebinar extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    $logger = $this->logger->get('WebformFormAPIHandlerForWebinar - postSave');
    if ($update) {
      $logger->notice('WebformFormAPIHandlerForWebinar - updatesubmission');
    }
    $logger->notice('WebformFormAPIHandlerForWebinar - create submission');
    ...
    $this->processFormDataWebinar($webform_submission, $getAccessToken['response'], $logger, "Webinar", $this->requestStack->fromOptions(), $apiKeys);
  }
   ...
   *   Boolean.
   */
  private function processFormDataWebinar(WebformSubmission $webform_submission, array $getAccessToken, LoggerChannel $logger, string $from_type, Client $requestStack, array $apiKeys): bool {
    $body_array_returned = $this->fieldToApiDataParsingWebinar($webform_submission, $logger, $from_type);
    $body_array_returned_json = Json::encode($body_array_returned);
    $logger->info("Form Data JSON Webinar- " . $body_array_returned_json);
    try {
      ...
      $logger->info("Form Data API Response Webinar- " . $form_data_api_response);
      ...
      $logger->info("Webinar - RequestException - " . $e->__toString() . "-getBody-" . $e->getMessage());
    }
    return TRUE;
  }
   ...
   *   Body Composed Array.
   */
  private function fieldToApiDataParsingWebinar(WebformSubmission $webform_submission, LoggerChannel $logger, string $from_type): array {
    ...
    $medicare_field_data_mapping_AOR_MAPD_exploded = explode(PHP_EOL, $bcbsma_webform_medicare_forms_api->get('medicare_field_data_mapping_webinar'));
