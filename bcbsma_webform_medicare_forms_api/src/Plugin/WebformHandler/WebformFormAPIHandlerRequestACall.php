 ...
 *
 * @WebformHandler(
 *   id = "medicare_forms_api_request_a_call",
 *   label = @Translation("MMSS - Medicare Forms API Request A Call"),
 ...
 * )
 */
class WebformFormAPIHandlerRequestACall extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    $logger = $this->logger->get('WebformFormAPIHandlerForRequestACall - postSave');
    if ($update) {
      $logger->notice('WebformFormAPIHandlerForRequestACall - updatesubmission');
    }
    $logger->notice('WebformFormAPIHandlerForRequestACall - create submission');
    ...
    $this->processFormDataRequestaCall($webform_submission, $getAccessToken['response'], $logger, "RequestACall", $this->requestStack->fromOptions(), $apiKeys);
  }
   ...
   *   Boolean.
   */
  private function processFormDataRequestaCall(WebformSubmission $webform_submission, array $getAccessToken, LoggerChannel $logger, string $from_type, Client $requestStack, array $apiKeys): bool {
    $body_array_returned = $this->fieldToApiDataParsingRequestaCall($webform_submission, $logger, $from_type);
    $body_array_returned_json = Json::encode($body_array_returned);
    $logger->info("Form Data JSON RequestACall- " . $body_array_returned_json);
    try {
      ...
      $logger->info("Form Data API Response RequestACall- " . $form_data_api_response);
      ...
      $logger->info("RequestACall - RequestException - " . $e->__toString() . "-getBody-" . $e->getMessage());
    }
    return TRUE;
  }
   ...
   *   Body Composed Array.
   */
  private function fieldToApiDataParsingRequestaCall(WebformSubmission $webform_submission, LoggerChannel $logger, string $from_type): array {
    ...
    $medicare_field_data_mapping_AOR_MAPD_exploded = explode(PHP_EOL, $bcbsma_webform_medicare_forms_api->get('medicare_field_data_mapping_request_a_call'));
