 ...
 *
 * @WebformHandler(
 *   id = "medicare_forms_api_campaign",
 *   label = @Translation("MMSS - Medicare Forms API Campaign & GuideBook Registration"),
 ...
 * )
 */
class WebformFormAPIHandlerCampaign extends WebformHandlerBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $from_type_value['Campaign'] = "Campaign";
    $from_type_value['Webpage'] = "Webpage";
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    $logger = $this->logger->get('WebformFormAPIHandlerForCampaign - postSave');
    if ($update) {
    ...
    $this->processFormDataCampaign($webform_submission, $getAccessToken['response'], $logger, $this->configuration['from_type'], $this->requestStack->fromOptions(), $apiKeys);
  }
   ...
   *   Boolean.
   */
  private function processFormDataCampaign(WebformSubmission $webform_submission, array $getAccessToken, LoggerChannel $logger, string $from_type, Client $requestStack, array $apiKeys): bool {
    $body_array_returned = $this->fieldToApiDataParsingCampaign($webform_submission, $logger, $from_type);
   ...
   *   Body Composed Array.
   */
  private function fieldToApiDataParsingCampaign(WebformSubmission $webform_submission, LoggerChannel $logger, string $from_type): array {
    ...
    $medicare_field_data_mapping_AOR_MAPD_exploded = explode(PHP_EOL, $bcbsma_webform_medicare_forms_api->get('medicare_field_data_mapping_campaign'));
    foreach ($medicare_field_data_mapping_AOR_MAPD_exploded as $val) {
      ...
      if ($val_explode['0'] == 'aep') {
        $body_composed_array['formData'][$val_explode['0']] = \Drupal::config('bcbsma_webform_medicare_forms_api.general_config')->get('enable_address_field_in_campaign') ? 'YES' : 'NO';
      }
      if ($val_explode['0'] == 'source') {
        $body_composed_array['formData'][$val_explode['0']] = $from_type;
      }
    }
