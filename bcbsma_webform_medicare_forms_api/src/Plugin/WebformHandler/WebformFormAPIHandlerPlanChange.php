 ...
 *
 * @WebformHandler(
 *   id = "medicare_forms_api_planchange",
 *   label = @Translation("MMSS - Medicare Forms API Plan Change"),
 ...
 * )
 */
class WebformFormAPIHandlerPlanChange extends WebformHandlerBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $from_type_value['CHANGE_PLAN_MEDEX'] = "CHANGE_PLAN_MEDEX";
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    $logger = $this->logger->get('WebformFormAPIHandlerForBillChange - postSave');
    if ($update) {
    ...
    $this->processFormDataPlanChange($webform_submission, $getAccessToken['response'], $logger, $this->configuration['from_type'], $this->requestStack->fromOptions(), $apiKeys);
  }
   ...
   *   Boolean.
   */
  private function processFormDataPlanChange(WebformSubmission $webform_submission, array $getAccessToken, LoggerChannel $logger, string $from_type, Client $requestStack, array $apiKeys): bool {
    $body_array_returned = $this->fieldToApiDataParsingPlanChange($webform_submission, $logger, $from_type);
   ...
   *   Body Composed Array.
   */
  private function fieldToApiDataParsingPlanChange(WebformSubmission $webform_submission, LoggerChannel $logger, string $from_type): array {
    ...
    $medicare_field_data_mapping_AOR_MAPD_exploded = explode(PHP_EOL, $bcbsma_webform_medicare_forms_api->get('medicare_field_data_mapping_planchange'));
    foreach ($medicare_field_data_mapping_AOR_MAPD_exploded as $val) {
      ...
      if ($val_explode['0'] == "currentPlan") {
        $plans = $webform_submission->getElementData(trim($val_explode['1']));
        $body_composed_array['formData'][$val_explode['0']] = $plans['current_plans'];
        $body_composed_array['formData']['changePlanRequestTo'] = $plans['updated_plans'];
        continue;
      }
      if ($val_explode['0'] == "addOnHearingVision") {
        $body_composed_array['formData'][$val_explode['0']] = ucwords(str_replace('_', ' ', $webform_submission->getElementData(trim($val_explode['1']))));
        continue;
      }
      if ($val_explode['0'] == "acknowledgeBox") {
        $acknowledgeBox_field_classigication = explode("::", $val_explode['1']);
        if ($webform_submission->getElementData('are_you_filling_the_form_as_self_enrollee_or_caregiver') == 'Member') {
